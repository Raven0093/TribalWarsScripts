...
class Player{
    constructor() {
        ...
        this.units.away = TribalWarsHtmlParser.TroopsPage.getUnitsStructure();
    ...
    updateUnits(){
        for (var i = 0; i< this.villages.length; i++){
            ...
            this.units.away = TribalWarsHtmlParser.TroopsPage.sumUnits([this.units.away, this.villages[i].units.away]);
...
TribalWarsUnits.Config.SPEC.ALL = [TribalWarsUnits.Config.SPEC.OFF, TribalWarsUnits.Config.SPEC.DEF];
...
TribalWarsHtmlParser.TroopsPage.getAwayUnits = function(page){
    ...
    if(unitsAwayPage){
        unitsAway = TribalWarsHtmlParser.TroopsPage.unitsHtmlTableToUnits(unitsAwayPage);
    }
    return unitsAway;
};
...
TribalWarsHtmlParser.TroopsPage.getSuppotedPlayers = function(page){
    ...
    try{
        if(unitsAwayPage){
            var players = TribalWarsHtmlParser.findElements(unitsAwayPage, "[data-id]");
        ...
        HttpHelper.getPage(url).then(function(response) {
            ...
            for(var i = 0; i< villagesId.length ; i++){
                ...
                promises.push(HttpHelper.getPage(url).then(function(req){
                    ...
                    newVillage.setAwayUnits(TribalWarsHtmlParser.TroopsPage.getAwayUnits(req));
...
function convertToAplieData(player){
    ...
    returnData.push(player.tribeRank);
    returnData.push(player.globalRank);
    returnData.push(player.ra);
    returnData.push(player.rd);
    ...
    returnData.push(player.getSpec(TribalWarsUnits.Config.SPEC.IDS.MEDIUM_OFF));
    returnData.push(player.getSpec(TribalWarsUnits.Config.SPEC.IDS.SMALL_OFF));
    returnData.push(player.getSpec(TribalWarsUnits.Config.SPEC.IDS.NEW_OFF));
    returnData.push(player.getSpec(TribalWarsUnits.Config.SPEC.IDS.SPY));
    returnData.push(player.getSpec(TribalWarsUnits.Config.SPEC.IDS.OFF_LK));
    returnData.push(player.getSpec(TribalWarsUnits.Config.SPEC.IDS.DEF_CK));
    returnData.push(player.getSpecs([
        TribalWarsUnits.Config.SPEC.IDS.DEF_SPEAR_SWORD,
        TribalWarsUnits.Config.SPEC.IDS.DEF_SPEAR_ARCHER,
        TribalWarsUnits.Config.SPEC.IDS.DEF_SPEAR_CK,
    ]));
